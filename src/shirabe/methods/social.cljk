(ns shirabe.methods.social
  "social.cljc — shirabe DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from shirabe's own
...
(defn draft-observation-post
  "A dry-run observation post over shirabe's own domain (Live research-concierge membrane — answers a natural-language question by planning → reading the read-only public web → grounding a…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "shirabe R0: live social posting is Council Lv6+ + operator + "
